...
def ask_quit(self):
    if messagebox.askokcancel("Okay to exit application?"):
        self.master.destroy()
        os._exit(0)
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_students (col_fname,col_lname,col_phone,col_email,col_course) VALUES (?,?,?,?,?)""",)
            conn.commit()
    conn.close()
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email,col_course FROM tbl_students WHERE col_course = (?)""", [value])
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            cursor.execute("""SELECT COUNT(col_course) FROM tbl_students WHERE col_course = '{}'""".format(var_course))#,(var_fullname))
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                self.lstList1.insert(END, var_fname, var_lname, var_phone, var_email, var_course) # update listbox with the new fullname
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                with conn:
                    cursor = conn.cursor()
                    cursor.execute("""DELETE FROM tbl_students WHERE col_course = '{}'""".format(var_select))
...
def onRefresh(self):
    ...
    with conn:
        ...
        i = 0
        while i < count:
                cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email,col_course FROM tbl_students""")
